...
class MAMBA4D(nn.Module):
    def __init__(self, radius, nsamples, spatial_stride,                                # P4DConv: spatial
        ...
        if intra:
            self.tube_embedding = IntraMamba(**feature_extraction_params)
        else:
            feature_extraction_params = dict(feature_extraction_params)
            feature_extraction_params.pop('depth_mamba_intra')
            self.tube_embedding = P4DConv(**feature_extraction_params)
    ...
    def forward(self, input):                                                                                                               # [B, L, N, 3]
        ...
        if features.dim() == 4 and features.shape[2] == self.pos_embedding.out_channels:
